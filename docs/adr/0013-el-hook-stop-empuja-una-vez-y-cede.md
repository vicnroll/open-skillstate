...
Se decide **un solo empujón**: bloquea la primera vez, y si vuelve a entrar con `stop_hook_active: true`, cede. Quien lo evalúa es el propio binario, invocado como `skstate hook stop` ([ADR 0012](./0012-init-completa-la-instalacion.md)); no hay script intermedio con lógica propia.
