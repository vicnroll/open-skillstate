...
El problema se evita de raíz: si el Σ de los workers nunca entra en Git, integrar worktrees no puede producir conflictos sobre él, y el único camino de fusión sigue siendo `skstate merge`.

## Consequences

- **Los dos Σ necesitan nombres distintos.** Los worktrees comparten el `.gitignore` del repositorio, así que una misma ruta relativa no puede estar ignorada en el worktree y versionada en la copia principal. El estado efímero del worker y el consolidado del orquestador tienen que vivir en ficheros con nombres diferentes dentro de `.openskillstate/`.
...
- **Versionado con un *merge driver* propio** (`.gitattributes` con `merge=skstate`) — descartado pese a ser el mecanismo que Git prevé para este caso: un driver no registrado en un clon hace que Git caiga **en silencio** en la fusión textual. Un fallo silencioso hacia el modo incorrecto es exactamente lo que se ha evitado en todas las demás decisiones.
